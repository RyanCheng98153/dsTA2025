...
function sendLetter(call, callback) {
    ...
    console.log("server.js > sendLetter")
    // 印出寄送信件的資訊
    // 格式如下：
    ...
    // 請參考 trackLetter 的實作，完成 sendLetter 的 callback 實作 trackLetter을 참고해서 sendLetter의 callback을 완성해라
    // Hint: 這裡需要回傳學生姓名、學院名稱、追蹤碼 여기는 학생 이름, id를 반환
    // Hint: 可以使用 call.request.<變數名稱> 來取得來自 client 的變數，
    // <變數名稱>可以參考 proto 檔案中的定義。변수이름은 프로토 파일에 정의
    ...
    /*if(letters[tracking_id]){
        callback(null,{
            ...
            status: letters[traking_id].status
...
function trackLetter(call, callback) {
    ...
    console.log("server.js > trackLetter")
