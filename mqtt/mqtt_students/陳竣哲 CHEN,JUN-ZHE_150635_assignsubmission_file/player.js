...
client.on('message', (topic, message) => {
  ...
  if (topic === 'battle/action') {
    // 接收由 Boss 發出的回合資訊 (回合數與剩餘血量)
    let data;
    try {
      data = JSON.parse(msg);
    } catch(err) {
      console.error('無法解析回傳的訊息:', err);
      return;
    }
    // 檢查回傳的訊息中是否包含回合數與 Boss 剩餘血量的資訊
    if (data.round !== undefined && data.boss_hp !== undefined) {
      console.log(`[ 回合 ${data.round} ]: Boss 剩餘血量 ${data.boss_hp}`);
      
      // 若 Boss 血量小於等於 0，則顯示擊敗訊息並結束連線
      if (data.boss_hp <= 0) {
