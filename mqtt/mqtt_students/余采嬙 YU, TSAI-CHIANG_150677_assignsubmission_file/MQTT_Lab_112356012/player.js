...
client.on('message', (topic, message) => {
  ...
  if (topic === 'battle/online') {
    ...
    client.publish('battle/action', JSON.stringify(attackData));
    // === 請實作這裡(以上) ===
  ...
  if (topic === 'battle/action') {
    const bossUpdateMsg = JSON.parse(msg);

    if (bossUpdateMsg.round !== undefined) {
      console.log(`[ 回合 ${bossUpdateMsg.round} ]: Boss 剩餘血量 ${bossUpdateMsg.boss_hp}`);

      if (bossUpdateMsg.boss_hp <= 0) {
