...
client.on('message', (topic, message) => {
  ...
  if (topic === 'battle/online') {
    ...
    client.publish('battle/action', JSON.stringify(attackMessage));
    // === 請實作這裡(以上) ===
  ...
  if (topic === 'battle/action') {
    try {
      ...
      const { round, boss_hp } = action;
      if (round !== undefined) {
        console.log(`[ 第 ${round} 回合 ]: Boss 剩餘血量 ${boss_hp}`);
      ...
      console.error('錯誤：無法解析 action 訊息內容', error);
    }
  }
  // === 請實作這裡(以上) ===
});
