...
client.on('connect', () => {
  ...
  // 可參考 Game.js
  client.subscribe(['battle/action', 'battle/online'], function (err) {
    if (err) {
      console.log('Subscription error:', err);
    } else {
      console.log('成功訂閱至主題 battle/action, battle/online');
    }
  });
  // === 請實作這裡(以上) ===
...
client.on('message', (topic, message) => {
  ...
  if (topic === 'battle/online') {
    ...
    client.publish('battle/action', JSON.stringify(attackStatus));
  ...
  if (topic === 'battle/action'){
    ...
    if (action.round !== undefined) {
      console.log(`[ 第 ${action.round} 回合 ]`);
      console.log(`Boss 受到 ${PLAYER_NAME}的攻擊 ,損失${attack_value}HP ,剩餘HP: ${action.boss_hp}`);
